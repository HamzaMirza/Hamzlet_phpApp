			...
			for(let i=0;i<data.clothes.length;i++)
			{
				if(data.clothes[i].MF!=2)
				{
			        ...
			        document.getElementById('menHolder').appendChild(getli(data.clothes[i].C_NAME.toUpperCase(),data.clothes[i].C_ID,'m'));
					 ...
					 document.getElementById('listholder').appendChild(getli(data.clothes[i].C_NAME.toUpperCase(),data.clothes[i].C_ID,'f'));
				...
				else if(data.clothes[i].MF==2)
				{
				  ...
				  { 
						type: 'GET', 
						url: 'http://localhost/rip/services/getWomenClothes.php',  
						dataType: 'json',
						success: function (data)
						{
							for(let i=0;i<data.clothes.length;i++)
								womensClotheslist.push(data.clothes[i]);
